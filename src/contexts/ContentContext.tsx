...
export function ContentProvider({ children }: { children: React.ReactNode }) {
  ...
  // Don't use cache on initial load - always fetch fresh from Supabase
  // This ensures the website always shows the latest content
  console.log('🔄 Will fetch fresh content from Supabase...');
  ...
  // Subscribe to real-time content updates from Supabase
  useEffect(() => {
    let unsubscribe: (() => void) | null = null;

    try {
      setLoading(true);
  console.log('🔄 Subscribing to Supabase content updates...');
      
      // Subscribe to real-time updates from Firestore
      unsubscribe = ContentService.subscribeToContent((newContent) => {
  console.log(`✅ Received ${newContent.length} content items from Supabase`);
  ...
  setError('Failed to connect to Supabase. Using cached data.');
      setLoading(false);
    }
    ...
    return () => {
      if (unsubscribe) {
  console.log('🔌 Unsubscribing from Supabase updates');
        unsubscribe();
      }
    };
  }, []);
  ...
  console.log(`✅ Loaded ${contentData.length} items from Supabase`);
      ...
      try {
        ...
        if (cached) {
          ...
          if (cachedData.length > 0) {
            ...
            setError('Using cached content. Please check your Supabase connection.');
            return;
          }
        }
      } catch (cacheErr) {
  ...
  setError('Could not load content from Supabase. Please check your connection.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const updateContent = async (
    ...
    try {
      // Update in Firestore
  console.log(`🔄 Updating '${id}' in Supabase...`);
  ...
  setError('Failed to update content in Supabase. Please try again.');
      throw error;
    }
  };
  ...
  console.log(`🗑️ Deleting '${id}' from Supabase...`);
      await ContentService.deleteContent(id);
  console.log(`✅ Successfully deleted '${id}' from Supabase`);
  ...
  setError(err.message || 'Failed to delete content from Supabase');
      throw err;
    }
  };
